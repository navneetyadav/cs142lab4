...
//elements entry for the array
void enqueue(int value)
	{
	//if there is no queue
	if(n==0){cout << "Queue is empty" << endl; return;}

	//assigning the values of 1st array to 2nd array
	for(int i=0;i<n;i++){
		b[i] = a[i];
	}

	//filling the 0th index of the array and entering the rey of the elements of original array
	a[0] = value;
	for(int i=0;i<n-1;i++){
		a[i+1] = b[i];
	}
...
//deleting the elements of the queue from the front(obvious)
void dequeue(){
